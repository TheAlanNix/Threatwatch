...
@section('content')
	...
	@else
		...
		@if (!empty(getSetting('THREATSTREAM_KEY')))
			<div class="col-xs-12">
				<h3>Anomali ({{ $threat_stream_data->meta->total_count }} Entries)</h3>

				@if ($threat_stream_data->meta->total_count > 0)

					<?php $i = 0 ?>

					@foreach ($threat_stream_data->objects as $object)

						<?php

							switch ($object->meta->severity) {
								case "low":
									$severity_color = "#5cb85c";
									break;
								case "medium":
									$severity_color = "#f0ad4e";
									break;
								case "high":
								case "very-high":
									$severity_color = "#d9534f";
							}
						...
						<div class="col-xs-12 col-md-6">
							<div class="col-xs-12 box-shadow buffer">
								<div class="col-xs-4 center">
									<h4>ThreatScore</h4>
									<div class="circle-chart" data-percent="{{ $object->threatscore }}"></div>
								</div>
								<div class="col-xs-4 center">
									<h4>Confidence</h4>
									<div class="circle-chart" data-percent="{{ $object->confidence }}"></div>
								</div>
								<div class="col-xs-4 center">
									<h4>Severity</h4>
									<div style="padding-top: 20px; color: {{ $severity_color }}">{{ strtoupper($object->meta->severity) }}</div>
								</div>
								<div class="col-xs-12">
									<table class="table table-striped">
										<tr>
											<td>Status</td>
											<td style="color: {{ $status_color }}">{{ strtoupper($object->status) }}</td>
										</tr>
										<tr>
											<td>Threat Type</td>
											<td>{{ strtoupper($object->threat_type) }} ({{ strtoupper($object->type) }})</td>
										</tr>
										<tr>
											<td>Threat Value</td>
											<td>{{ $object->value }}</td>
										...
										<tr>
											<td>Geolocation</td>
											<td><a target="_blank" href="http://maps.google.com/?ll={{ $object->latitude }},{{ $object->longitude }}">Google Maps</a></td>
										</tr>
										<tr>
											<td>Source</td>
											<td>{{ $object->source }}</td>
				...
				@else
					<div class="col-xs-12 center">No Anomali data for this IP</div>
				@endif
